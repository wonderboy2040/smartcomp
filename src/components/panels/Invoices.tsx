...
export function PdfPreview({ url, onClose }: { url: string; onClose: () => void }) {
  ...
  const currentUrl = url + (url.includes('?') ? '&' : '?') + `template=${selectedTemplate}&banner=${bannerStyle}`
      ...
      <div className="bg-white rounded-2xl w-full max-w-6xl h-[95vh] flex flex-col shadow-2xl border overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-3 bg-slate-50 border-b overflow-x-auto flex-shrink-0">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2 text-[10px]">
            <span className="px-2 py-1 bg-white border border-slate-200 rounded-full text-slate-600 font-semibold">Ad Banner:</span>
            {AD_BANNER_VARIANTS.map((v) => (
              <button
                key={v.id}
                onClick={() => setBannerStyle(v.id)}
                title={v.description}
                className={`px-2.5 py-1 rounded-full border text-[10px] font-bold transition-all ${bannerStyle === v.id ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}
              >
                {v.name}
              </button>
            ))}
